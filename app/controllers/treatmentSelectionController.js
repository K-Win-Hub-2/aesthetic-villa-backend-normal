  ...
  try {
    ...
    if (latestDocument.length > 0 && latestDocument[0].seq) {
      ...
      req.body["VoucherCode"] =
        "TVC-" + today[0].replace(/-/g, "") + "-" + increment; // if seq is undefined set initial patientID and seq
    ...
    const filterLevel = memLevelRes.filter(
      (el) => el.totalAmount <= parseInt(patientData[0].totalPaidAmount)
    );
    ...
    const resultUpdate = await Patient.findOneAndUpdate(
      ...
      {
        totalAmount:
          parseInt(patientData[0].totalPaidAmount) +
          parseInt(data.totalPaidAmount),
    ...
    const tsupdate = await Patient.findOneAndUpdate(
      ...
      { $inc: { totalTS: TSArray.length, unfinishedTS: TSArray.length } }
    );
    if (tvcCreate === true) {
    ...
    //update paid amount in patient
    console.log("tota", totalPaidAmount, paidAmount, secondAmount);
    await Patient.findByIdAndUpdate(relatedPatient, {
      $inc: {
        totalAmount: totalAmount,
        totalPaidAmount: Number(totalPaidAmount) + (Number(secondAmount) || 0),
      },
    });
